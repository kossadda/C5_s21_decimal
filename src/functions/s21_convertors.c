...
int s21_from_decimal_to_int(s21_decimal src, int *dst) {
    ...
    return (src.bits[1] || src.bits[2] || dst == NULL) ? 1 : 0;
}
...
int s21_from_float_to_decimal(float src, s21_decimal *dst) {
    ...
    if (!(s21_isinf(src) || s21_isinf(-src) || s21_isnan(src) || src == 0)) {
        ...
        if (length_int_part < 7) {
            ...
            for (int i = 0; i < length_int_part - 7; i++) {
                if (i == length_int_part - 8)
                    round_val = ((unsigned int)src) % 10;
                src /= 10;
            }
            res = truncf(src);
            if (round_val > 5) {
    ...
    return (s21_isinf(src) || s21_isinf(-src) || s21_isnan(src) ||
            (res == 0 && src))
               ? 1
               : 0;
}
...
int s21_from_decimal_to_float(s21_decimal src, float *dst) {
    ...
    return ((!decimal_is_empty(src) && small_value) || dst == NULL) ? 1 : 0;
}