...
int s21_mul(s21_decimal value_1, s21_decimal value_2, s21_decimal *result) {
    ...
    big_decimal_mul(small_decimal_to_big(value_1),
                    small_decimal_to_big(value_2), &res_big);
    if (scale) {
        while (!big_decimal_is_empty(res_big)) {
            if (!scale) {
                break;
            } else if (!(res_big.bits[4] && res_big.bits[5] &&
                         res_big.bits[6])) {
            ...
            *result = value_1;
            ...
            if (scale1 != scale2) {
                for (int i = 0; i < scale1 + scale2; i++) {
                    if (i < scale2)
                        big_decimal_mul(
                            val1_big, (s21_big_decimal){{10, 0, 0, 0, 0, 0, 0}},
                            &val1_big);
                    if (i < scale1)
                        big_decimal_mul(
                            val2_big, (s21_big_decimal){{10, 0, 0, 0, 0, 0, 0}},
                            &val2_big);
                }
            }
